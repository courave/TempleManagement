...
namespace CWCS_OL.tables.stock.instock
{
    public partial class edit : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    if (!this.GetCurrentUser().hasPermission("s2c"))
                    {
                ...
                {
                    if (!this.GetCurrentUser().hasPermission("s2u"))
                    {
                    ...
                    using (DataBase db = new DataBase())
                    {
                        ...
                        TextBox_rukudate.Text = Server.HtmlEncode(dr[16].ToString());
                        TextBox_jiaokushu.Text = Server.HtmlEncode(dr[17].ToString());
                        TextBox_danjia.Text = Server.HtmlEncode(dr[18].ToString());
                        TextBox_caiwu.Text = Server.HtmlEncode(dr[19].ToString());
                        TextBox_lianxuhao.Text = Server.HtmlEncode(dr[20].ToString());
                        TextBox_jizhang.Text = Server.HtmlEncode(dr[21].ToString());
                        TextBox_baoguan.Text = Server.HtmlEncode(dr[22].ToString());
                        TextBox_yanshou.Text = Server.HtmlEncode(dr[23].ToString());
                        TextBox_fapiao.Text = Server.HtmlEncode(dr[24].ToString());
                    }
                }
            }
        ...
        protected String SaveNew()
        {
            ...
            using (DataBase db = new DataBase())
            {
                ...
                //RUKU_DATE,JIAOKUSHU,DANJIA,CAIWU,LIANXUHAO,JIZHANG,BAOGUAN,YANSHOU,FAPIAO
                DateTime tmp = DateTime.MinValue;
                DateTime.TryParse(TextBox_rukudate.Text, out tmp);
                if (tmp == DateTime.MinValue)
                    db.AddParameter("RUKU_DATE", DBNull.Value);
                else
                    db.AddParameter("RUKU_DATE", tmp);
                db.AddParameter("JIAOKUSHU", TextBox_jiaokushu .Text);
                db.AddParameter("DANJIA", TextBox_danjia.Text);
                db.AddParameter("CAIWU", TextBox_caiwu.Text);
                db.AddParameter("LIANXUHAO", TextBox_lianxuhao.Text);
                db.AddParameter("JIZHANG", TextBox_jizhang.Text);
                db.AddParameter("BAOGUAN", TextBox_baoguan.Text);
                db.AddParameter("YANSHOU", TextBox_yanshou.Text);
                db.AddParameter("FAPIAO", TextBox_fapiao.Text);
                String sql;
                sql = "INSERT INTO [StockTrans] ([Type],[StockId],[GoodsId],[BillNo],[EditTime],[Amount],[TotalPrice],[FromTo],[Manager],[Operator],[Remark]"+
                    ",[RUKU_DATE],[JIAOKUSHU],[DANJIA],[CAIWU],[LIANXUHAO],[JIZHANG],[BAOGUAN],[YANSHOU],[FAPIAO]) " +
                    "VALUES (0,@StockId,@GoodsId,@BillNo,@EditTime,@Amount,@TotalPrice,@FromTo,@Manager,@Operator,@Remark"+
                    ",@RUKU_DATE,@JIAOKUSHU,@DANJIA,@CAIWU,@LIANXUHAO,@JIZHANG,@BAOGUAN,@YANSHOU,@FAPIAO)";
        ...
        protected bool SaveOld(String id)
        {
            ...
            using (DataBase db = new DataBase())
            {
                ...
                sql = "UPDATE [StockTrans] SET [BillNo] = @BillNo, [TotalPrice]=@TotalPrice, " +
                    ...
                    "[Remark]=@Remark,[RUKU_DATE]=@RUKU_DATE,[JIAOKUSHU]=@JIAOKUSHU,"+
                    "[DANJIA]=@DANJIA,[CAIWU]=@CAIWU,[LIANXUHAO]=@LIANXUHAO,[JIZHANG]=@JIZHANG,"+
                    "[BAOGUAN]=@BAOGUAN,[YANSHOU]=@YANSHOU,[FAPIAO]=@FAPIAO WHERE [ID]=@ID";
            ...
            else
            {
                ...
                else
                {
                    using (DataBase db = new DataBase())
                    {
                        ...
                        DataTable dt = db.ExecuteDataTable("SELECT TOP 1 [ID] FROM [Goods] WHERE [ID] > @ID AND [Type] = 0 ORDER BY [ID] ASC");
                        if (dt.Rows.Count == 1)
                        {
