...
namespace CWCS_OL.tables.stock.instock
{
    public partial class detail : System.Web.UI.Page
    {
        ...
        {
            if (!this.GetCurrentUser().hasPermission("s2r"))
            {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    db.AddParameter("ID", Request.Params["id"]);
                    DataTable dt = db.ExecuteDataTable("SELECT TOP 1 [ID] FROM [StockTrans] WHERE [ID]<@ID AND [Type]=0 ORDER BY [ID] DESC");
                    if (dt.Rows.Count == 1)
                    {
                ...
                {
                    db.AddParameter("ID", Request.Params["id"]);
                    DataTable dt = db.ExecuteDataTable("SELECT TOP 1 [ID] FROM [StockTrans] WHERE [ID]>@ID AND [Type]=0 ORDER BY [ID] ASC");
                    if (dt.Rows.Count == 1)
                    {
