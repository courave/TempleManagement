...
namespace CWCS_OL.tables.stock.change
{
    public partial class detail : System.Web.UI.Page
    {
        ...
        {
            if (!this.GetCurrentUser().hasPermission("s4r"))
            {
            ...
            if (!IsPostBack)
            {
                ...
                using (DataBase db = new DataBase())
                {
                    ...
                    Label_Category.Text = Server.HtmlEncode(dr[6].ToString());
                ...
                {
                    db.AddParameter("ID", Request.Params["id"]);
                    DataTable dt = db.ExecuteDataTable("SELECT TOP 1 [ID] FROM [StockTrans] WHERE [ID]<@ID AND [Type]=2 ORDER BY [ID] DESC");
                    if (dt.Rows.Count == 1)
                    {
