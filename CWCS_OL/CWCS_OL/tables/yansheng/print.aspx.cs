...
namespace CWCS_OL.tables.yansheng
{
    public partial class print : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                Session["yansheng_reportsource"] = null;
                InitTemplateList();
            }
            if (Session["yansheng_reportsource"] == null)
            {
                InitPage();
            }
            else
            {
                CrystalReportViewer1.ReportSource = Session["yansheng_reportsource"];
            }
        }
        ...
        protected void InitTemplateList()
        {
            using (DataBase db = new DataBase())
            {
                ...
                DataTable dt = db.ExecuteDataTable("SELECT [id], [name], [deftemplate] FROM [PrintConfig] WHERE [type] IN (N'yansheng',N'yansheng_single')");
                String strDefault = "";
                ...
                if (Session["yansheng_printtemplate"] == null)
                {
                    if (strDefault != "")
                    {
                ...
                {
                    DropDownList_template.SelectedValue = Session["yansheng_printtemplate"].ToString();
                }
            }
        }
        ...
        {
            List<yanshengPrint> l = new List<yanshengPrint>();
            ...
            if (Request.Params["printall"] != null)
            {
                using (DataBase db = new DataBase())
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        MarkAsPrinted(dr[0].ToString(), "tbYANSHENG");
                    }
                }
            }
            else if (Request.Params["fromPage"] != null)
            {
                using (DataBase db = new DataBase())
                {
                    ...
                    DataTable dt = db.ExecuteDataTable("SELECT [ID], [LOG_TIME], [LOG_USER], " +
                        "[FAHUI_NAME], [ZHUZHAO1], [ZHUZHAO2], [ZHUZHAO3], [ZHUZHAO4], [ZIHAO]," +
                        " [ZUOCI] FROM [tbYANSHENG] WHERE [ZUOCI] BETWEEN '" +
                        fromPage + "' AND '" + toPage + "'");
                    if (dt.Rows.Count < 1)
                    {
                        Response.End();
            ...
            {
                if (Session["yansheng_cursql"] == null)
                    Response.End();
                using (DataBase db = new DataBase())
                {
                    DataTable dt = db.ExecuteDataTable(Session["yansheng_cursql"].ToString());
                    foreach (DataRow dr in dt.Rows)
                    {
                        l.Add(GetPrintByDataRow(dr));
                        MarkAsPrinted(dr[0].ToString(), "tbYANSHENG");
            ...
            else
            {
                ...
                DataTable dtOne = db2.ExecuteDataTable("SELECT [ID],[LOG_TIME],[LOG_USER],[FAHUI_NAME],[ZHUZHAO1],[ZHUZHAO2],[ZHUZHAO3],[ZHUZHAO4],[ZIHAO],[ZUOCI] FROM [tbYANSHENG] WHERE [ID] = @ID");
            ...
            ReportClass cr_yansheng = new CrystalReport_yansheng();

            if (DropDownList_template.SelectedValue == "0")
            {
                cr_yansheng = new CrystalReport_yansheng();
            }
            else if (DropDownList_template.SelectedValue == "-1")
            {
                cr_yansheng=new CrystalReport_yansheng_single();
            }
            else
            {
                using(DataBase db=new DataBase()){
                    ...
                    if (dt.Rows.Count != 0)
                        cr_yansheng = CWCS_OL.print.PrintEdit.GetNewReportByType(dt.Rows[0][0].ToString());
                    else
                        cr_yansheng = new CrystalReport_yansheng();
                }
            }
            cr_yansheng.SetDataSource(l);
            Session["yansheng_reportsource"] = cr_yansheng;
            CrystalReportViewer1.ReportSource = cr_yansheng;
        }

        private yanshengPrint GetPrintByDataRow(DataRow dr)
        {
            yanshengPrint yp = new yanshengPrint();
        ...
        {
            Session["yansheng_printtemplate"] = DropDownList_template.SelectedValue;
            InitPage();
        }
    }
}