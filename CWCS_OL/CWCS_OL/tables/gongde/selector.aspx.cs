    ...
    public partial class selector : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.Params["type"]!=null && Request.Params["fangwei"] != null)
            {
                ...
                {
                    sql = "select rows,cols from gongde_type where type='" + Request.Params["type"] +
                        "' and fangwei='" + Request.Params["fangwei"]+"'";
                    ...
                    foreach (string a in cols)
                    {
                        ...
                        for (int i = 1; i <= c; i++)
                        {
                            ...
                            sql = "SELECT SHIZHU_NAME FROM SHIZHU_GONGDE A,SHIZHU_INFO B WHERE A.SHIZHU_ID=B.SHIZHU_ID AND "+
            ...
            else if (Request.Params["type"] != null && Request.Params["fangwei"] == null)
            {
                using (DataBase db = new DataBase())
                {
                    sql = "select fangwei from gongde_type where type='"+Request.Params["type"]+"'";
                    dt = db.ExecuteDataTable(sql);
