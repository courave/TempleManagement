        ...
        if (context.Request.Files.Count > 0)
        {
            ...
            for (int j = 0; j < context.Request.Files.Count; j++)
            {
                ...
                if (uploadFile.ContentLength > 0)
                {
                    ...
                    if (context.Session["filemgr_editid"] == null) break;
                    using (DataBase db = new DataBase())
                    {
                        byte[] fileData = new byte[uploadFile.ContentLength];
                        uploadFile.InputStream.Read(fileData, 0, fileData.Length);
                        db.AddParameter("MgrId", context.Session["filemgr_editid"].ToString());
                        db.AddParameter("FileName", uploadFile.FileName);
                        context.Session["FileNames"] += uploadFile.FileName + "[br]";
                        db.AddParameter("Data", fileData);
